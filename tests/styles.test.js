...
describe('[src/styles.js tests]', () => {
    ...
    before(async () => {
        ...
        assert.equal(
            styleDefinition.featureStyles.length,
            drawingInfo.renderer.uniqueValueInfos.length + 1
        );
    ...
    it('should create feature style', async () => {
        const style = await createFeatureStyle(
            styleDefinition.featureStyles[1]
        );
        ...
        assert.equal(
            ...
            `rgba(${esriColorToOLColor(drawingInfo.renderer.uniqueValueInfos[1].symbol.color).join()})`
        );
        assert.isArray(style.getStroke().getLineDash());
        assert.equal(style.getStroke().getLineDash()[0], 10);
        assert.equal(
            style.getStroke().getWidth(),
            drawingInfo.renderer.uniqueValueInfos[1].symbol.width
        );
